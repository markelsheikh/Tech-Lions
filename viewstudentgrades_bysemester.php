...
function view_gradesbysemester_render($message=null,$error=null ,$grades_result=null){
		...
		$dbcon->close();
        echo $twig->render('viewstudentgrades_bysemester.html',array('user' => $user_data,'term'=>$term_result,'grades'=>$grades_result,'term_selected'=>$term_id,'message'=>$message,'error'=>$error,'student'=>$student_result));
}
...
function view_grades(){
		...
		$sql = "SELECT COURSE.CRS_TITLE ,ENROLL.MID_TERM_GRADE, ENROLL.ENROLL_GRADE FROM COURSE JOIN ENROLL ON COURSE.CRS_CODE = ENROLL.CRS_CODE WHERE ENROLL.TERM_ID = $term AND ENROLL.STU_ID = $student_id";
